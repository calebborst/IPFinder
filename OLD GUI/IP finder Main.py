...
#-=-Changable-=-#
#Window Creation#
title_A = "IPFinder: 0.8.0"#		<-- Title of Customization Window
title_B = "Debug Menu"#				<-- Title of Debug (Output) Window
version_title = "Version: 0.8.0"#	<-- Version Number
...
stop = end + Yellow + "[" + Red + "||" + Yellow + "]"
start = end + Yellow + "[" + Green + "|>" + Yellow + "]"
detect_os = end + Cyan + "[" + Yellow + "<>" + Cyan + "]"
#Main Loop#
myip = get('http://myip.dnsomatic.com/').text
ipstart = 0#					<-- Defines Whether the IpFinder is On Start of Program {True = On, False = Off}
writelinenum = 0
looper = 0
counter = 0
queue = 0
...
blacklist = 0#						<-- Use Blacklist file?
hiddenips = 1#						<-- 'Weed Out' check is the IP a Proxy or Vpn? 
usewindows = 0#						]
...
#Other#
time_metric = time.localtime()
date_metric = date.today()
current_time = time.strftime("%H:%M:%S", time_metric)
log_time = time.strftime("%H;%M;%S", time_metric)
log_date = date_metric.strftime('%d-%m-%y')
log_definer = '-d-' + log_date + '-t-' + log_time 
howmanydonefsofar = 0
...
def generate_dbg_buttons():
	#Output Window#
	#output_window = tk.Label(dbg_window,
	#						)
...
def ipfinderbegintrigger():
	...
	print(start + 'Program Starting')
	print(start +'IpSave: {},\nFastLoad:{},\nWorkerspeed: {},\nKeep: {},\nTarget Contry: {},\nClean Menu: {},\nGet Hoast: {},\nDisplay Contry: {}'.format(saveipstofile, fastloader, workerspeed, keep, countryname, cleant, gethostname, getcountry))

	with open('log' + log_definer + '.txt', 'a') as log:
		log.write('\nThe IP finder started at [' + current_time + ']\n\n')
		log.write('IpSave: {},\nFastLoad:{},\nWorkerspeed: {},\nKeep: {},\nTarget Contry: {},\nClean Menu: {},\nGet Hoast: {},\nDisplay Contry: {}\n\n'\
...
def ipfinderendtrigger():
	...
	begin_button.place(relx = 0.8, rely = 0.85, anchor = N)
	ipstart = 0
	print(stop + 'Program Stopped - (It May Take a second depending on Threadcount)')
	with open('log' + log_definer + '.txt', 'a') as log:
		log.write('The IP finder finished at [' + current_time + ']')
		log.close()
...
def final_checker():
	...
	global blacklist
	global hiddenips
	global govermentiptypes
	global proxyiptypes
	global toriptypes
	global glitch

	try:
		blklst = open("blacklist.txt", "r")
		blacklist = 1
	except:
		blacklist = 0

	if platform == "linux" or platform == "linux2":
		print(detect_os + "Detected linux OS.")
		uselinux = 1
	elif platform == "darwin":
		print(detect_os + "Detected Mac OS")
		usemac = 1
		print(Bad + "Unsaported atm...")
	elif platform == "win32":
		print(detect_os + "Detected Windows OS")
		usewindows = 1


	if usewindows == 1 and usemac == 0 and uselinux == 0:
		with open('log' + log_definer + '.txt', 'a') as log:
			log.write('\n[Program Opened on Windows-OS]\n')
			log.close()
	elif usewindows == 0 and usemac == 0 and uselinux == 1:
		with open('log' + log_definer + '.txt', 'a') as log:
			log.write('\n[Program Opened on Windows OS]\n')
			log.close()
	elif usewindows == 0 and usemac == 1 and uselinux == 0:
		with open('log' + log_definer + '.txt', 'a') as log:
			log.write('\n[Program Opened on Mac-OS]\n This OS is inconpatable. Program Terminated.\n')
			log.close()
		glitch = True
		end_prog()
	else:
		with open('log' + log_definer + '.txt', 'a') as log:
			log.write('\nThere was an Error detecting OS, Program Terminated\n')
			log.close()
		glitch = True
		end_prog()
	...
	if ipstart != 1:
		...
		if hiddenips >= 1:
			...
			if randomip in govermentiptypes:
				print("Found IP in goverment IP types. Generating a new one! IP was = " + randomip)
				updatable_government_ip = randomip
				randomip = socket.inet_ntoa(struct.pack('>I', random.randint(1, 0xffffffff)))
				with open('log' + log_definer + '.txt', 'a') as log:
					log.write('A Government IP was found on IP number: ' + str(counter_plus_1) + ', the IP was: ' + str(updatable_government_ip) + ', the new IP generated was: ' + str(randomip) + '.\n')
					log.close()
				return
			elif randomip in proxyiptypes: 
				print("Found IP in proxy IP types. Generating a new one! IP was = " + randomip)
				updatable_proxy_ip = randomip
				randomip = socket.inet_ntoa(struct.pack('>I', random.randint(1, 0xffffffff)))
				with open('log' + log_definer + '.txt', 'a') as log:
					log.write('A Proxy IP was found on IP number: ' + str(counter_plus_1) + ', the IP was: ' + str(updatable_proxy_ip) + ', the new IP generated was: ' + str(randomip) + '.\n')
					log.close()
				return
			elif randomip in toriptypes:
				print("Found IP in tor IP types. Geerating a new one! IP was = " + randomip)
				updatable_tor_ip = randomip
				randomip = socket.inet_ntoa(struct.pack('>I', random.randint(1, 0xffffffff)))
				with open('log' + log_definer + '.txt', 'a') as log:
					log.write('A Tor IP was found on IP number: ' + str(counter_plus_1) + ', the IP was: ' + str(updatable_tor_ip) + ', the new IP generated was: ' + str(randomip) + '.\n')
					log.close()
				return
	
		if blacklist >= 1:
			if randomip in blacklistedips:
				print("Found a black listed ip! Generating a new one! IP was = " + randomip)
				randomip = socket.inet_ntoa(struct.pack('>I', random.randint(1, 0xffffffff)))
				return
	
		#sets console title to the queue count (only if your on windows)
		if usewindows == 1:
			settitle = 'title Press ctrl + c to stop :: IPs scanned: ' + str(howmanydonefsofar) + ' :: Saved: ' + str(counter) + ' :: Queue = ' + str(queue) + ' :: ' + randomip
			os.system(settitle)
		...
		if saveipstofile >= 1:
			try:
				...
				with open('log' + log_definer + '.txt', 'a') as log:
					log.write('\nNo foundips.txt file found. Making new one.\n\n')
					log.close()
				createfoundips = open('foundips.txt', 'a')
				readips = open('foundips.txt', 'r')
		...
		if "255.255.255" in randomip:
			...
			randomip = socket.inet_ntoa(struct.pack('>I', random.randint(1, 0xffffffff)))
			with open('log' + log_definer + '.txt', 'a') as log:
				log.write('A 255.255.255 IP was found on IP number: ' + str(counter_plus_1) + ', the new IP generated was: ' + str(randomip) + '.\n')
				log.close()
			return
		if randomip == myip:
			...
			randomip = socket.inet_ntoa(struct.pack('>I', random.randint(1, 0xffffffff)))
			with open('log' + log_definer + '.txt', 'a') as log:
				log.write('Your IP was found on IP number: ' + str(counter_plus_1) + '... FuN, the new IP generated was: ' + str(randomip) + '.\n')
				log.close()
			return
		...
		if first == "10":
			...
			updatable_lan_ip = randomip
			randomip = socket.inet_ntoa(struct.pack('>I', random.randint(1, 0xffffffff)))
			with open('log' + log_definer + '.txt', 'a') as log:
				log.write('A 10. LAN IP was found on IP number: ' + str(counter_plus_1) + ', the IP was: ' + str(updatable_lan_ip) + ', the new IP generated was: ' + str(randomip) + '.\n')
				log.close()
			return
		if first == "172":
			if int(second) <= 15:
				...
				randomip = socket.inet_ntoa(struct.pack('>I', random.randint(1, 0xffffffff)))
				with open('log' + log_definer + '.txt', 'a') as log:
					log.write('A 172. LAN IP was found on IP number: ' + str(counter_plus_1) + ', the IP was: ' + str(updatable_lan_ip) + ', the new IP generated was: ' + str(randomip) + '.\n')
					log.close()
				return
			else:
				pass
		if first+'.'+second == "192.168":
			...
			randomip = socket.inet_ntoa(struct.pack('>I', random.randint(1, 0xffffffff)))
			with open('log' + log_definer + '.txt', 'a') as log:
				log.write('A 192.968. LAN IP was found on IP number: ' + str(counter_plus_1) + ', the IP was: ' + str(updatable_lan_ip) + ', the new IP generated was: ' + str(randomip) + '.\n')
				log.close()
			return
...
ipstart = 0
while True:
	if threadA.is_alive() is False and restart_windows != True:
		end_prog()
	if ipstart != 1:
		pass
	else:
