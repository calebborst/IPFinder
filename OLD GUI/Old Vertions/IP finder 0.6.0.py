...
#Window Creation#
title_A = "IPFinder: 0.6.0"#		<-- Title of Customization Window
...
getcountry = 0#						<--
usewindows = 0#						]
uselinux = 0#						[<-- Defult Os Detection State Updaten On Program Start
usemac = 0#							]

#Other#
ipstart = False#					<-- Defines Whether the IpFinder is On Start of Program {True = On, False = Off}
version_title = "Version: 0.6.0"#	<-- Version Number
...
#-=-Tickboxes-=-#
	...
	ma_target_var = tk.IntVar()#	[<-- Tick Box
	ma_hostgrab_var = tk.IntVar()#	[<-- Variable creation
...
def main_menu():
	...
	tb_fast_mode.pack()
	tb_fast_mode.place(relx = 0.3, rely = 0.35, anchor = W)	
	tb_target.pack()
...
def ipfinderbegintrigger():
	...
	clean_check()
	DB_check()
	#print(saveipstofile, fastloader, workerspeed, keep, contryname.get(), cleant, gethostname)
...
def fast_check():
	...
	if ma_fast_mode_var.get() == 1:
		wb_thread_count.pack()
		wb_thread_count.place(relx = 0.35, rely = 0.35, anchor = W)
		fastloader = 1
		try:
...
def target_check():
	...
	if ma_target_var.get() == 1:
		keep = 1
		wb_country.pack()
		wb_country.place(relx = 0.35, rely = 0.4, anchor = W)
		if resp_contry.get() != '':#or doesnt match contry list (need to impliment this)
			contryname = resp_contry
		else:
			keep = 0
